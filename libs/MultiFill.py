...
def userCustom(order, data, length, pos, str_sel):
    if (order == 'ordered'):
        ...
        print(data[random.randint(0, length - 1)])
        print(time.strftime("%Y-%m-%d %H:%M:%S"))
        return time.strftime(data[0])
    elif (order == 'replace'):
        s = ''
        for i in range(0, len(str_sel)):
            s += random.choice('abcdefghijklmnopqrstuvwsyz_-')
...
class TestMultiFillSetTextCommand(sublime_plugin.TextCommand):

    def run(self, edit, **args):
        print("test this is atrue")
        print("chosen = ", args.get("chosen"))
        print("custom = ", args.get("custom"))
        ...
        count = len(points)
        substr = self.view.substr(points[0])
        print("substr = ", substr)
...
class TestMultiFillCommand(sublime_plugin.WindowCommand):

    def run(self):
        # get the fill lists
        # self.command_list = getList() + self.get_custom()
        self.command_list = self.get_custom()
    ...
    def on_done(self, index):
        print("test on_done,index = ", index)
        # if (index == -1):
        # print("index == -1")
        # return
        # call the text filla
        ...
        if view:
            print("is active view")
            view.run_command('test_multi_fill_set_text', {
                             'chosen': index, 'custom': self.settings.get("custom")})
        # return
...
#   formula insert part
#
class TestMultiIntegerSetTextCommand(sublime_plugin.TextCommand):

    def run(self, edit, **args):
        print("this is test multi integer set text")
        ...
        for x in range(0, count):
            try:
                ...
                echo(
                    "Sorry! Parse error, MultiFill dose not support pure math-like formula yet.")
                return
...
class TestMultiIntegerCommand(sublime_plugin.WindowCommand):

    def run(self):
        self.window.show_input_panel(
            "Multi Integer formula: ", "y = ", self.on_done, None, None)
...
class TestMultiSelectWindowsCommand(sublime_plugin.WindowCommand):
    def run(self, **args):
...
class TestMultiMoveWindowCommand(sublime_plugin.WindowCommand):
    def run(self, **args):
        dir = args.get('direction')
...
class TestMultiWindowCompareCommand(sublime_plugin.WindowCommand):
    def run(self, **args):
        dir = args.get('direction')
        sync = args.get('sync')
        view = self.window.active_view()
        ...
        (region_start_row, region_start_col) = view.rowcol(
            view.visible_region().begin())
        (region_end_row, region_end_col) = view.rowcol(
            view.visible_region().end())
        ...
        if (sync == 'true'):
            point = view.text_point(
                (region_start_row + (region_end_row - region_start_row) / 2), 0)
...
class TestCloseOtherTabsCommand(sublime_plugin.TextCommand):
    def run(self, edit):
